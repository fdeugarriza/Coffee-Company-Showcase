...
-- ORDER BY, LIMIT, DISTINCT and RENAMING
...
-- Return email and length of characters
SELECT email, 
...
-- Concatenate first and last names to make full names
SELECT first_name, last_name, employee_id,
first_name || ' ' || last_name AS full_name
FROM employees;

-- Concatenate columns to make sentences
...
-- If the person makes less than 50k and is female, then true, otherwise false
SELECT first_name || ' ' || last_name AS full_name,
salary, 
gender,
...
-- If email address has '.com' return true, otherwise false
SELECT email, 
...
-- COALESCE to fill missing value with custom value
SELECT email,
COALESCE(email, 'NO EMAIL PROVIDED')
FROM employees;

-- Let's count the number of emails provided
SELECT COUNT (email)
FROM employees;
...
SELECT MAX(salary) AS min_sal
FROM employees;

-- Show the difference between miimum and maximum salry
...
-- Let's round the numbers to the nearest integer
...
-- Return the number of employees, the average, the minimum, the maximum and total salaries for each coffeeshop
...
-- After GROUP BY, return only the coffeeshops in which the minimum salary is less than 10k
SELECT coffeeshop_id,
COUNT(*) AS num_of_emp,
ROUND(AVG(salary), 0) AS avg_sal,
MIN(salary) AS min_sal,
...
-- If pay is lass then 20k, then low pay. If pay between 20k and 50k inclusive, then medium pay. If salary is over 50k, then high pay
SELECT 
employee_id, 
first_name, 
last_name,
...
-- JOIN & UNION
-- Insert values for the JOIN exercise
...
-- Delete the values inserted for the JOIN exercise
DELETE FROM locations WHERE city_id = 4;
DELETE FROM shops WHERE coffeeshop_id = 6;

-- UNION
-- Return all the cities and countries
...
-- Return all coffeeshop names, all cities and all countries
...
-- Return the maximum salary from all shops
SELECT
first_name,
last_name,
salary,
(SELECT MAX(salary) AS max_sal FROM employees LIMIT 1)
FROM employees;

-- Return the average salary from all shops
SELECT
first_name,
last_name,
salary,
(SELECT ROUND(AVG(salary), 0) AS avg_sal FROM employees LIMIT 1)
FROM employees;

-- Show the difference between average salary and each worker's salary
SELECT
first_name,
last_name,
salary,
salary - (SELECT ROUND(AVG(salary), 0) AS avg_sal FROM employees LIMIT 1)
FROM employees;
...
-- Return all employees who work in US shops
SELECT *
FROM employees
...
SELECT 
  hire_date,
  salary,
  SUM(salary) OVER (ORDER BY hire_date ROWS BETWEEN 30 PRECEDING AND CURRENT ROW) AS pay_pattern
FROM employees
ORDER BY hire_date;
